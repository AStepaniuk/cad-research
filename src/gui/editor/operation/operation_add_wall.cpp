...
action_handle_status operation_add_wall::handle_rollback()
{
    _document.model.points().erase(_current_point.value());
    _current_point = std::nullopt;
...
action_handle_status operation_add_wall::handle_mouse_move(float mx, float my)
{
    return _sub_operation_move_handle.handle_mouse_move(mx, my);
}

action_handle_status operation_add_wall::handle_left_mouse_click(float mx, float my)
{
    _sub_operation_move_handle.stop();
