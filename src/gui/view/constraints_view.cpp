...
void constraints_view::render(ImDrawList *draw_list)
{
    ...
    {
        draw_constraint(draw_list, c.second, Styles::CColor);
    }

    for (const auto& c : _document.active_wall_snaps.constraints())
    {
        draw_constraint(draw_list, c.second, Styles::CSnapColor);
    }
}

void constraints_view::draw_constraint(ImDrawList* draw_list, const floor::constraint_t &c, ImU32 color)
{
